...
pack()
{
  ...
  $msbuild -t:restore "$rootdir/src/Package/CppSharp.Runtime.csproj" -p:Configuration=$configuration -p:Platform=$platform
  $msbuild -t:pack "$rootdir/src/Package/CppSharp.Runtime.csproj" -p:Configuration=$configuration -p:Platform=$platform -p:PackageOutputPath="$rootdir/artifacts"
}
